...
const combinationInputTemplate = document.querySelector(
".combination-input-template");

const combinationInputContainer = document.getElementById(
...
unlockButton.onclick = () => {
  if (checkCombination()) {
    startUnlockAttempt(() => {
      const tl = new TimelineMax({ onComplete: finishUnlockAttemptSuccess });
      lockRestingColor = "hsl(120,50%,100%)";
      responseText.innerHTML = "access granted";
    ...
    startUnlockAttempt(() => {
      const tl = new TimelineMax({ onComplete: finishUnlockAttemptFail });
      lockRestingColor = "hsl(0,50%,100%)";
      responseText.innerHTML = "denied";
...
const startUnlockAttempt = onComplete => {
  ...
  "#unlock-button",
  0.5,
  { y: 550, opacity: 0, ease: Power2.easeInOut },
  "a");

  tl.staggerTo(
  ".combination-input",
  0.5,
  { y: 200, opacity: 0, ease: Power2.easeInOut },
  0.1,
  "a+=0.25");
